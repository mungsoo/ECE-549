...
def get_scale_space(input, num_sigma, min_sigma, sigma_ratio, octave_size, method="downsample"):
    ...
    if method == "normal":
        ...
        # for i in range(num_sigma):
            # scale_space[:, :, i] = (gaussian_images[i] - gaussian_images[i+1]) * sigma_list[i]
        ...
        for i in range(num_sigma):
            if i != 0 and i % octave_size == 0:
                gaussian_image = skimage.transform.resize(gaussian_image, (gaussian_image.shape[0] \
                                        // 2, gaussian_image.shape[1] // 2), anti_aliasing=True)
                cur_sigma = min_sigma
                cur_gaussian = gaussian_filter(gaussian_image, cur_sigma)
            ...
            cur_gaussian = gaussian_filter(gaussian_image, cur_sigma)
            scale_space[:, :, i] = skimage.transform.resize(cur_gaussian - pre_gaussian, input.shape, anti_aliasing=False)
        # print("gogodsfdassssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss")
...
blob_detection(butterfly, num_sigma, method="dog", threshold=threshold, nms_size=nms_size)

# aa = gaussian_filter(gaussian_filter(butterfly, 2), (1.259 - 1) * 2)
# bb = gaussian_filter(butterfly, 2 * 1.259)
# print(aa-bb)
# print(aa)